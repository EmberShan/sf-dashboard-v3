...
export default function GlobalHeader({
  ...
  return (
    <header
      className="w-full flex flex-col gap-2 border-b border-gray-200 bg-background-color z-50 px-8 py-4"
      style={{ minHeight: 80 }}
    >
        ...
        <div className="flex flex-wrap gap-2 items-center ml-8">
              ...
              <DropdownMenu key={dropdown.key}>
                ...
                <DropdownMenuContent align="start" closeOnSelect={false}>
                  <DropdownMenuCheckboxItem
                    ...
                    onCheckedChange={() => handleMultiSelect(dropdown.key, "__ALL__")}
          ...
          <div className="flex items-center gap-1 rounded-md font-medium text-sm mx-2">
            ...
            <input
              ...
              onChange={(e) => setFilters((f) => ({ ...f, price: { ...f.price, min: e.target.value } }))}
            ...
            <input
              ...
              onChange={(e) => setFilters((f) => ({ ...f, price: { ...f.price, max: e.target.value } }))}
              className="w-16 border rounded-md px-1 py-1.5 bg-white p-4 shadow-xs hover:bg-gray-50"
              placeholder="Max"
            />
          </div>
          ...
          <div className="flex items-center gap-1 font-medium text-sm ml-2">
            ...
            <input
              ...
              onChange={(e) => setFilters((f) => ({ ...f, cost: { ...f.cost, min: e.target.value } }))}
              className="w-16 border rounded-md px-1 py-1.5 bg-white p-4 shadow-xs hover:bg-gray-50"
              placeholder="Min"
            />
            <span className="text-xs">-</span>
            <input
              ...
              onChange={(e) => setFilters((f) => ({ ...f, cost: { ...f.cost, max: e.target.value } }))}
              className="w-16 border rounded-md px-1 py-1.5 bg-white p-4 shadow-xs hover:bg-gray-50"
              placeholder="Max"
            />
          </div>
          ...
          <div className="flex items-center gap-1 font-medium text-sm ml-2">
            ...
            <input
              ...
              onChange={(e) => setFilters((f) => ({ ...f, margin: { ...f.margin, min: Number(e.target.value).toFixed(1) } }))}
              className="w-16 border rounded-md px-1 py-1.5 bg-white p-4 shadow-xs hover:bg-gray-50"
              placeholder="Min"
            />
            <span className="text-xs">-</span>
            <input
              ...
              onChange={(e) => setFilters((f) => ({ ...f, margin: { ...f.margin, max: Number(e.target.value).toFixed(1) } }))}
              className="w-16 border rounded-md px-1 py-1.5 bg-white p-4 shadow-xs hover:bg-gray-50"
              placeholder="Max"
            />
          </div>
